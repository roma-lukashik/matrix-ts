...
import { isMatrixN, matrixn, ndim, size } from '../geometry'
import { add, broadcast, divide, MatrixBinaryOperator, multiply } from '../binary-operation'
...
>(matrix: T1, [d0, ...dn]: T2, operator: MatrixBinaryOperator): T3 => {
  if (isMatrixN(matrix) && isDefined(d0)) {
